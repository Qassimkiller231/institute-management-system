...
export default function TakeTestPage() {
  ...
  /**
   * Effect 2: Save Progress
   * Auto-save test answers and current question to localStorage
   * Allows resuming test if page refreshes
   */
  useEffect(() => {
   ...
   * Counts down time and auto-submits when time runs out
   */
  useEffect(() => {
  ...
  /**
   * Start a new test session or resume existing one
   * Handles various scenarios:
   * - New session creation
   * - Already completed test (show results)
   * - Test with no questions (error)
   */
  const startTest = async () => {
    ...
    try {
      ...
      if (!sessionResult.success) {
        ...
        if (sessionResult.message?.includes("already taken") || sessionResult.message?.includes("already have")) {
          try {
            ...
            // console.error("Error fetching last session:", e); // Debug
          }
        }
      ...
      // console.error("Start test error:", err); // Debug
      setError(`Error: ${err.message || "Network error. Please try again."}`);
    } finally {
      setLoading(false);
    }
  ...
  /**
   * Submit test answers
   * Saves session ID for speaking test booking
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        setSubmitted(true);
        saveTestSessionId(sessionId); // Save for booking speaking test
  ...
  /**
   * Parse question options from JSON string or array
   * Handles both database formats
   */
  const parseOptions = (options: any): string[] => {
    ...
    try {
      ...
      // console.error("Error parsing options:", error); // Debug
      return [];
    }
  ...
  /**
   * Test selection screen (main)
   */
  const renderTestSelection = () => {
    if (loading) {
      return <LoadingState message="Loading Tests..." submessage="Please wait" />;
    }
  ...
  /**
   * Test submitted successfully state
   */
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4">
        <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center">
          ...
          <p className="text-gray-700 mb-6 text-lg">
            Great job! To complete your placement test, please book a speaking test appointment.
          </p>
          <button
            onClick={() => router.push("/book-speaking")}
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        <div className="mt-4 bg-gray-200 rounded-full h-2">
          <div
            ...
            style={{
              width: `${((currentQuestion + 1) / test.totalQuestions) * 100}%`,
            }}
          />
        </div>
      </div>
    ...
    return (
      <div className="space-y-3">
        {options.map((option: any, index: number) => {
          ...
          // Handle both object {text, ...} and string formats
  ...
  /**
   * Question card with question text and answer options
   */
  ...
  /**
   * Test taking screen (main)
   */
  const renderTestTaking = () => {
    if (loading && !test) {
      return <LoadingState message="Loading Test..." submessage="Please wait" />;
    }
