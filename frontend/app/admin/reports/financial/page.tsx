  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-700">View financial analytics and revenue reports</p>
      </div>

      {/* Filters */}
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Select Term</h2>
        ...
        <div className="space-y-6">
          ...
          <div className="grid md:grid-cols-4 gap-6">
            <div className="bg-white rounded-lg shadow p-6 border-l-4 border-green-600">
              ...
              <p className="text-3xl font-bold text-gray-900">{reportData.revenue?.totalCollected?.toFixed(2) || 0} BD</p>
              <p className="text-xs text-gray-600 mt-1">All payments collected</p>
            </div>

            <div className="bg-white rounded-lg shadow p-6 border-l-4 border-blue-600">
              <p className="text-sm font-medium text-gray-700 mb-2">Expected Revenue</p>
              <p className="text-3xl font-bold text-gray-900">{reportData.revenue?.totalExpected?.toFixed(2) || 0} BD</p>
              <p className="text-xs text-gray-600 mt-1">Total expected from plans</p>
            </div>

            <div className="bg-white rounded-lg shadow p-6 border-l-4 border-yellow-600">
              <p className="text-sm font-medium text-gray-700 mb-2">Pending Payments</p>
              <p className="text-3xl font-bold text-gray-900">{reportData.revenue?.pending?.toFixed(2) || 0} BD</p>
              <p className="text-xs text-gray-600 mt-1">Outstanding amount</p>
            </div>

            <div className="bg-white rounded-lg shadow p-6 border-l-4 border-purple-600">
              <p className="text-sm font-medium text-gray-700 mb-2">Collection Rate</p>
              <p className="text-3xl font-bold text-gray-900">{reportData.collectionRate?.toFixed(1) || 0}%</p>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="grid md:grid-cols-4 gap-4">
              {reportData.paymentMethods && Object.entries(reportData.paymentMethods).map(([method, amount]: [string, any]) => (
                <div key={method} className="bg-gray-50 p-4 rounded-lg">
                  <p className="text-sm font-medium text-gray-700">{method.replace('_', ' ')}</p>
                  <p className="text-2xl font-bold text-gray-900 mt-1">{amount.toFixed(2)} BD</p>
                </div>
              ))}
            </div>
          </div>
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-right text-sm font-semibold text-gray-900">Expected</th>
                    <th className="px-6 py-3 text-right text-sm font-semibold text-gray-900">Difference</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {reportData.monthlyTrend?.map((month: any, idx: number) => (
                    <tr key={idx} className="hover:bg-gray-50">
                      <td className="px-6 py-4 text-sm font-medium text-gray-900">{month.month}</td>
                      <td className="px-6 py-4 text-sm text-right text-gray-900">{month.collected.toFixed(2)} BD</td>
                      <td className="px-6 py-4 text-sm text-right text-gray-900">{month.expected.toFixed(2)} BD</td>
                      <td className="px-6 py-4 text-sm text-right">
                        <span className={`font-semibold ${month.difference >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {month.difference >= 0 ? '+' : ''}{month.difference.toFixed(2)} BD
                        </span>
                      </td>
                    </tr>
                  ))}
      ...
      {!reportData && !loading && (
        <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-12 text-center">
          <p className="text-gray-600 text-lg">Select term and click "Generate Report" to view financial data</p>
        </div>
      )}
    </div>
  );
