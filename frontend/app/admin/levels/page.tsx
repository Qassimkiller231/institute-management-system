  ...
  const renderHeader = () => (
    <div className="mb-6 flex justify-between items-center">
      ...
      <div className="flex gap-3">
        <button onClick={openCreateModal} className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700">+ Create Level</button>
      </div>
    </div>
  );

  const renderTableRow = (level: Level) => (
  ...
  const renderTable = () => (
    <div className="bg-white rounded-lg shadow overflow-hidden">
      <table className="w-full">
        ...
        <tbody className="divide-y divide-gray-200">{levels.map(renderTableRow)}</tbody>
      </table>
    </div>
  );
