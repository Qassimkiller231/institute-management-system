...
import { groupsAPI, termsAPI, levelsAPI, teachersAPI, venuesAPI, programsAPI } from '@/lib/api';
...
export default function CreateGroupPage() {
  ...
  const renderActions = () => (
    <div className="flex gap-4 pt-4">
      <button type="button" onClick={() => router.back()} className="flex-1 px-6 py-3 border-2 border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50" disabled={loading}>Cancel</button>
      <button type="submit" className="flex-1 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50" disabled={loading}>{loading ? 'Creating...' : 'Create Group'}</button>
    </div>
  );
