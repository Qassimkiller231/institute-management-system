  ...
  const renderTableRow = (plan: any) => {
    ...
    return (
      <tr key={plan.id} className="hover:bg-gray-50">
        <td className="px-6 py-4 text-sm font-medium text-gray-900">{plan.enrollment?.student?.firstName} {plan.enrollment?.student?.secondName}</td>
        <td className="px-6 py-4 text-sm text-gray-900">{plan.enrollment?.group?.groupCode || 'N/A'}</td>
        <td className="px-6 py-4 text-center"><span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${paidInstallments === totalInstallments ? 'bg-green-100 text-green-800' : paidInstallments > 0 ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>{paidInstallments}/{totalInstallments}</span></td>
        ...
        <td className="px-6 py-4 text-center"><span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${status === 'Fully Paid' ? 'bg-green-100 text-green-800' : status === 'In Progress' ? 'bg-blue-100 text-blue-800' : status === 'Overdue' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'}`}>{status}</span></td>
        <td className="px-6 py-4 text-center"><div className="flex justify-center gap-2"><button onClick={() => handleViewInstallments(plan)} className="px-3 py-1 bg-purple-600 text-white text-xs rounded hover:bg-purple-700">View</button><button onClick={() => handleEdit(plan)} className="px-3 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700">Edit</button><button onClick={() => handleDelete(plan)} className="px-3 py-1 bg-red-600 text-white text-xs rounded hover:bg-red-700">Delete</button></div></td>
      </tr>
    );
  };
    ...
    <div className="bg-white rounded-lg shadow overflow-hidden">
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-200">{filteredPlans.map(renderTableRow)}</tbody>
        </table>
      </div>
      {filteredPlans.length === 0 && <div className="p-12 text-center"><p className="text-gray-600">No payment plans found</p></div>}
    </div>
  );
  ...
  const renderEditModal = () => !showEditModal ? null : (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <p className="text-sm text-gray-600 mb-4">Student: <strong>{selectedPlan?.enrollment?.student?.firstName} {selectedPlan?.enrollment?.student?.secondName}</strong></p>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-900 mb-1">Total Amount (BD)</label>
            <input type="number" step="0.01" value={editForm.totalAmount} onChange={(e) => setEditForm({ ...editForm, totalAmount: parseFloat(e.target.value) })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-900" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-900 mb-1">Discount Amount (BD)</label>
            <input type="number" step="0.01" value={editForm.discountAmount} onChange={(e) => setEditForm({ ...editForm, discountAmount: parseFloat(e.target.value) })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-900" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-900 mb-1">Discount Reason</label>
            <input type="text" value={editForm.discountReason} onChange={(e) => setEditForm({ ...editForm, discountReason: e.target.value })} placeholder="e.g., Scholarship" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-900" />
          </div>
          <div className="p-3 bg-blue-50 rounded">
            <p className="text-sm text-gray-700"><strong>Final Amount:</strong> {(editForm.totalAmount - editForm.discountAmount).toFixed(2)} BD</p>
          </div>
        </div>
        <div className="flex gap-3 mt-6">
          <button onClick={submitEdit} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Update Plan</button>
          <button onClick={() => setShowEditModal(false)} className="flex-1 px-4 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400">Cancel</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl my-8">
        ...
        <div className="space-y-4">
          <div>
            ...
            <select value={createForm.enrollmentId} onChange={(e) => setCreateForm({ ...createForm, enrollmentId: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-900">
              <option value="">-- Select Enrollment --</option>
              {enrollments.map((enr: any) => <option key={enr.id} value={enr.id}>{enr.student?.firstName} {enr.student?.secondName} - {enr.group?.groupCode}</option>)}
            </select>
          </div>
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1">Total Amount (BD)</label>
              <input type="number" step="0.01" value={createForm.totalAmount} onChange={(e) => setCreateForm({ ...createForm, totalAmount: parseFloat(e.target.value) || 0 })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-900" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1">Discount Amount (BD)</label>
              <input type="number" step="0.01" value={createForm.discountAmount} onChange={(e) => setCreateForm({ ...createForm, discountAmount: parseFloat(e.target.value) || 0 })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-900" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-900 mb-1">Discount Reason</label>
            <input type="text" value={createForm.discountReason} onChange={(e) => setCreateForm({ ...createForm, discountReason: e.target.value })} placeholder="e.g., Scholarship, Early Bird" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-900" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-900 mb-1">Number of Installments</label>
            <input type="number" min="1" max="12" value={createForm.totalInstallments} onChange={(e) => setCreateForm({ ...createForm, totalInstallments: parseInt(e.target.value) || 1 })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-900" />
          </div>
          <div className="flex gap-3"><button onClick={generateInstallments} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Generate Installments</button></div>
          {createForm.installments.length > 0 && <div className="mt-4"><h3 className="font-semibold text-gray-900 mb-2">Generated Installments:</h3><div className="max-h-48 overflow-y-auto border-2 border-gray-300 rounded-lg p-3 bg-gray-50">{createForm.installments.map((inst: any, idx: number) => <div key={idx} className="flex justify-between py-2 px-1 text-sm font-medium text-gray-900 hover:bg-gray-100 rounded"><span>Installment #{inst.installmentNumber}</span><span>{inst.amount.toFixed(2)} BD - Due: {inst.paymentDate}</span></div>)}</div></div>}
  ...
  const renderInstallmentRow = (inst: any) => {
    ...
    return (
      <tr key={inst.id} className="hover:bg-gray-50">
        <td className="px-4 py-2 text-sm text-gray-900">{inst.installmentNumber}</td>
        ...
        <td className="px-4 py-2 text-sm text-center text-gray-900">{inst.paymentMethod || '-'}</td>
    ...
    <div className="mb-4 p-4 bg-blue-50 rounded border border-blue-200">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div>
          ...
          <input type="number" step="0.01" value={newInstallment.amount} onChange={(e) => setNewInstallment({ ...newInstallment, amount: parseFloat(e.target.value) || 0 })} className="w-full px-3 py-2 border border-gray-300 rounded text-gray-900" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-900 mb-1">Due Date</label>
          <input type="date" value={newInstallment.dueDate} onChange={(e) => setNewInstallment({ ...newInstallment, dueDate: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded text-gray-900" />
        </div>
        <div className="flex items-end gap-2">
          <button onClick={handleAddInstallment} className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Save</button>
          <button onClick={() => setShowAddInstallment(false)} className="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400">Cancel</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
      <div className="bg-white rounded-lg p-6 w-full max-w-5xl my-8">
        <div className="flex justify-between items-start mb-4">
          <h2 className="text-xl font-bold text-gray-900">Installments - {selectedPlan.enrollment?.student?.firstName} {selectedPlan.enrollment?.student?.secondName}</h2>
          <button onClick={() => setShowAddInstallment(!showAddInstallment)} className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 text-sm">+ Add Installment</button>
        </div>
        <div className="mb-4 p-4 bg-gray-50 rounded">
          <div className="grid md:grid-cols-3 gap-4 text-sm">
            <div><span className="text-gray-600">Total Amount:</span><span className="font-semibold ml-2 text-gray-900">{Number(selectedPlan.totalAmount).toFixed(2)} BD</span></div>
            <div><span className="text-gray-600">Discount:</span><span className="font-semibold ml-2 text-gray-900">{Number(selectedPlan.discountAmount || 0).toFixed(2)} BD</span></div>
            <div><span className="text-gray-600">Final Amount:</span><span className="font-semibold ml-2 text-gray-900">{Number(selectedPlan.finalAmount).toFixed(2)} BD</span></div>
          </div>
        </div>
        {renderAddInstallmentForm()}
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {selectedPlan.installments?.map(renderInstallmentRow)}
            </tbody>
          </table>
        </div>
        <div className="flex justify-end mt-6">
          <button onClick={() => setShowInstallmentsModal(false)} className="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400">Close</button>
        </div>
      </div>
    </div>
  );
