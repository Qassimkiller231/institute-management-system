  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-700">View and manage student payments and installments</p>
      </div>

      {/* Stats */}
      <div className="grid md:grid-cols-4 gap-6 mb-6">
        <div className="bg-white rounded-lg shadow p-6 border-l-4 border-blue-600">
          <p className="text-sm font-medium text-gray-700 mb-1">Total Installments</p>
          <p className="text-3xl font-bold text-gray-900">{stats.total}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-6 border-l-4 border-green-600">
          <p className="text-sm font-medium text-gray-700 mb-1">Paid</p>
          <p className="text-3xl font-bold text-gray-900">{stats.paid}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-6 border-l-4 border-yellow-600">
          <p className="text-sm font-medium text-gray-700 mb-1">Pending</p>
        ...
        <div className="grid md:grid-cols-3 gap-4 mt-4">
          ...
          <div>
            ...
            >
              ...
              {groups.map(group => (
                <option key={group.id} value={group.groupCode}>{group.groupCode}</option>
              ))}
            </select>
          </div>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-center text-sm font-semibold text-gray-900">Due Date</th>
                <th className="px-6 py-3 text-center text-sm font-semibold text-gray-900">Payment Date</th>
                <th className="px-6 py-3 text-center text-sm font-semibold text-gray-900">Method</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredPayments.map((payment) => (
                <tr key={payment.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-900">{payment.groupCode}</td>
                  <td className="px-6 py-4 text-sm text-center text-gray-900">{payment.installmentNumber}</td>
                  <td className="px-6 py-4 text-sm text-right font-semibold text-gray-900">{payment.amount.toFixed(2)} BD</td>
                  <td className="px-6 py-4 text-sm text-center text-gray-900">{payment.dueDate}</td>
                  <td className="px-6 py-4 text-sm text-center text-gray-900">{payment.paymentDate || '-'}</td>
                  <td className="px-6 py-4 text-sm text-center text-gray-900">{payment.paymentMethod || '-'}</td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <p className="text-sm text-gray-600 mb-4">
              ...
              Amount: <strong>{selectedPayment?.amount.toFixed(2)} BD</strong>
            </p>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-900 mb-1">Receipt Number</label>
                <input
                  type="text"
              ...
              {paymentForm.paymentMethod === 'BENEFIT_PAY' && (
                <div>
                  <label className="block text-sm font-medium text-gray-900 mb-1">Benefit Reference</label>
                  <input
                    type="text"
                    value={paymentForm.benefitReference}
                    onChange={(e) => setPaymentForm({...paymentForm, benefitReference: e.target.value})}
                    placeholder="BEN-2025-XXX"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-900"
                  />
                </div>
              )}
