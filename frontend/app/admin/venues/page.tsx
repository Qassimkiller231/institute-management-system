      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full">
            <h2 className="text-2xl font-bold mb-4">{modalMode === 'create' ? 'Create' : 'Edit'} Venue</h2>
            <div className="space-y-4">
              ...
              {modalMode === 'create' && (
                <div>
                  <label className="block text-sm font-medium mb-1">Venue Code *</label>
                  <input
                    type="text"
                    value={formData.code}
              ...
              <div>
                ...
                <input
                  type="text"
                  value={modalMode === 'create' ? formData.address : (editData.address || '')}
                  onChange={(e) => modalMode === 'create'
                    ? setFormData({ ...formData, address: e.target.value })
                    : setEditData({ ...editData, address: e.target.value })
                  }
                  className="w-full px-4 py-2 border rounded-lg text-gray-900"
                  placeholder="Building 123, Road 456"
                />
              </div>
            </div>
