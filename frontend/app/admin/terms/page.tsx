...
export default function TermsManagement() {
  ...
  const fetchTerms = async () => {
    try {
      ...
      const response = await termsAPI.getAll(isActiveParam);
      ...
      <div className="bg-white rounded-lg shadow mb-6 p-4">
        <div className="flex gap-4 items-center">
          ...
          <select
            ...
            className="px-4 py-2 border rounded-lg text-gray-900 min-w-[180px]"
          >
          ...
          <select
            ...
            className="px-4 py-2 border rounded-lg text-gray-900 min-w-[150px]"
          >
            ...
            {filteredTerms.map((term) => (
              <React.Fragment key={term.id}>
                ...
                >
                  ...
                  <td className="px-6 py-4 text-gray-700">{term._count?.groups || 0} groups</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={(e) => { e.stopPropagation(); handleToggle(term.id, term.isActive); }}
                      ...
                      {term.isCurrent && (
                        <span className="text-yellow-500 text-lg" title="Current Term">⭐</span>
                      )}
                      <button
                        onClick={(e) => { e.stopPropagation(); handleSetCurrent(term.id); }}
                        className="text-blue-600 hover:underline text-xs"
                      >
                        {term.isCurrent ? 'Unset' : 'Set Current'}
                      </button>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <button onClick={(e) => { e.stopPropagation(); openEditModal(term); }} className="text-blue-600 hover:underline mr-4">Edit</button>
                    {!term.isActive ? (
                      <button onClick={(e) => { e.stopPropagation(); handleReactivate(term.id, term.name); }} className="text-green-600 hover:underline">Reactivate</button>
                    ) : (
                      <button onClick={(e) => { e.stopPropagation(); handleDelete(term.id, term.name); }} className="text-red-600 hover:underline">Delete</button>
                    )}
                  </td>
                </tr>
                ...
                {expandedTerm === term.id && (
                  <tr>
                    <td colSpan={7} className="px-6 py-4 bg-gray-50">
                      <div className="ml-8">
                        <h4 className="font-semibold text-gray-900 mb-2">Groups in {term.name}:</h4>
                        {termGroups[term.id]?.length > 0 ? (
                          ...
                          <p className="text-sm text-gray-500">No groups in this term</p>
                        )}
                      </div>
                    </td>
                  </tr>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 bg-blue-50 p-4 rounded-lg">
                <input
                  type="checkbox"
                  id="isCurrent"
                  checked={modalMode === 'create' ? (formData.isCurrent || false) : (editData.isCurrent || false)}
                  onChange={(e) => modalMode === 'create'
                    ? setFormData({ ...formData, isCurrent: e.target.checked })
                    : setEditData({ ...editData, isCurrent: e.target.checked })
                  }
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    If checked, this will become the active term for the selected program. Any existing current term for this program will be unset.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3 bg-blue-50 p-4 rounded-lg">
