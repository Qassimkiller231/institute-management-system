...
import { STATIC_URL } from '@/lib/api/client';
...
export default function StudentManagement() {
  ...
  const renderStudentRow = (student: Student) => (
    <tr key={student.id} className="hover:bg-gray-50"><td className="px-6 py-4 whitespace-nowrap"><div className="w-10 h-10 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center">{student.profilePicture ? <img src={`${STATIC_URL}${student.profilePicture}`} alt={student.firstName} className="w-full h-full object-cover" /> : <svg className="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clipRule="evenodd" /></svg>}</div></td><td className="px-6 py-4 whitespace-nowrap"><div className="font-medium text-gray-900">{student.firstName} {student.secondName} {student.thirdName}</div></td><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.cpr}</td><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.user?.email || 'N/A'}</td><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.user?.phone || 'N/A'}</td><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.gender}</td><td className="px-6 py-4 whitespace-nowrap text-sm">{student.currentLevel ? <span className="px-2 py-1 bg-indigo-100 text-indigo-800 rounded-full text-xs font-semibold">{student.currentLevel}</span> : <span className="text-gray-400">Not Set</span>}</td><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.enrollments?.[0]?.group?.level?.name || 'N/A'}</td><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.enrollments?.[0]?.group?.venue?.name || 'N/A'}</td><td className="px-6 py-4 whitespace-nowrap"><span className={`px-2 py-1 text-xs rounded-full ${student.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{student.isActive ? 'Active' : 'Inactive'}</span></td><td className="px-6 py-4 whitespace-nowrap text-sm space-x-2"><button onClick={() => openEditModal(student)} className="text-blue-600 hover:text-blue-800">Edit</button>{student.isActive ? <button onClick={() => handleDelete(student.id, student.firstName)} className="text-red-600 hover:text-red-800">Delete</button> : <button onClick={() => handleReactivate(student.id, student.firstName)} className="text-green-600 hover:text-green-800">Reactivate</button>}</td></tr>
  );
