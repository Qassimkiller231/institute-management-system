    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600 mb-4">
          Amount: <strong>{parseFloat(installment.amount).toFixed(2)} BD</strong>
        </p>
      </div>
      ...
      <div className="flex gap-3">
        ...
        >
          {loading ? 'Processing...' : `Pay ${parseFloat(installment.amount).toFixed(2)} BD`}
        </button>
        <button
          type="button"
          onClick={onCancel}
          disabled={loading}
          className="px-6 py-3 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 disabled:bg-gray-100"
        >
          Cancel
        </button>
      </div>
